...
import exifr from 'exifr'
 ...
 export default function ImageResizer (files, config) {
    ...
    if (This.config.debug) {
        console.log('Initialised ImageResizer for ' + This.config.inputElement);
    }

};

ImageResizer.prototype.handleFileList = function(fileArray) {
    var This = this;
...
ImageResizer.prototype.scaleImage = function(img, completionCallback, orientation) {
	...
	if (mWidth<=0){
		mWidth = 1;
		console.warn('ImageResizer: image size is too small');
	}
    ...
    canvas.toBlob((blob)=>{this.progressObject.imgResized.push(blob); completionCallback();},'image/jpeg', this.config.quality);
	/*if (typeof this.config.onScale === 'function') {
        this.config.onScale(imageData);
    }
    this.progressObject.imgResized.push(imageData)
    console.log('fini ?')
    completionCallback()*/
    //this.performUpload(imageData, completionCallback);
};

ImageResizer.prototype.performUpload = function(imageData, completionCallback) {
...
ImageResizer.prototype.uploadComplete = function(event, completionCallback) {
...
ImageResizer.prototype.progressUpdate = function(itemDone, itemTotal) {
